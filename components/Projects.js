    ...
    <section id="projects" className="bg-primary py-20">
      <div className="max-w-6xl mx-auto px-6">
        <ScrollReveal>
          ...
          <div className="w-16 h-1.5 bg-dark rounded-full mb-10" />
        </ScrollReveal>

        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {projects.map((project, i) => (
            <ScrollReveal key={project.title} delay={0.1 * (i + 1)}>
              <TiltCard className="flex flex-col rounded-2xl bg-dark p-6 border border-white/10 h-full">
                <div className="flex items-start justify-between mb-3">
                  <h3 className="font-extrabold text-white text-xl leading-snug">
                    {project.title}
                  </h3>
                  {project.inProgress && (
                    <span className="text-xs bg-primary/20 text-primary font-bold px-2 py-1 rounded-full whitespace-nowrap ml-2">
                      In Progress
                    </span>
                  )}
                </div>

                <p className="text-white/70 text-base font-medium flex-1 mb-4">
                  {project.description}
                </p>
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {project.tech.map((tech) => (
                    <span
                      key={tech}
                      className="text-xs font-bold bg-primary/15 border border-primary/30 text-primary rounded-full px-3 py-1"
                    >
                      {tech}
                    </span>
                  ))}
                ...
                {project.github && (
                  <a
                    ...
                    className="inline-flex items-center gap-2 text-base font-bold text-white hover:text-primary transition-colors"
                  >
